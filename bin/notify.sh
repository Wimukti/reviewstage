...
notify_card() {
  ...
  payload=$(echo "$payload" | jq -c --arg k "$kind" --arg r "${REPO:-}" --argjson t "$(date +%s)" \
...
# Renders the same Block Kit cards the scripts used to build inline, word for word.
_notify_slack() {
  local p="$1" kind pr login blocks
  kind=$(echo "$p" | jq -r .kind); pr=$(echo "$p" | jq -r .pr); login=$(echo "$p" | jq -r .login)
  case "$kind" in
    review_requested)
      ...
      echo "$blocks" | slack_post "$pr" root "$login";;
    review_ready)
      ...
      blocks=$(echo "$p" | jq '
        ...
        {blocks:[
          {type:"section", text:{type:"mrkdwn",
            text:($w + $i + " Review ready — *<" + .url + "|#" + .pr + " — " + .title + ">*\n*"
      ...
      echo "$blocks" | slack_post "$pr" reply "$login";;
    review_stopped)
      ...
      blocks=$(echo "$p" | jq '
        (if (.extra.text // "") != "" then .extra.text
           ...
           "⚠️ Review of *<" + .url + "|#" + .pr + ">* failed: " + (.extra.message // "")
         else "🛑 Review of *<" + .url + "|#" + .pr + " — " + .title + ">* was stopped." end) as $t |
    ...
    qa_ready)
      blocks=$(echo "$p" | jq '
        ...
        {blocks:[
          {type:"section", text:{type:"mrkdwn",
            text:($w + "📋 QA guide ready — *<" + .url + "|#" + .pr + " — " + .title + ">*")}},
          {type:"actions", elements:[
