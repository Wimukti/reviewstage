...
One server serves the whole team. Each person signs in once; from then on their queue, their runs, their posts and their approvals are theirs.
...
| --- | --- |
| Poll review requests, clone the base repo | The **service token** in `.env`. Read-only. |
