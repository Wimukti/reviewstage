...
## W — website
- [x] Home rebuilt to the four sections in proposal §"Home": fold (≤ 25 words), strip (five
      states, keyboard-navigable, `role=tablist`), guarantees (five lines), install + tiers.
- [x] Hero = `StageScene state="staged" play` inside `StageFrame`, lit by §4.1, replay control.
- [x] Strip = `StageScene` / `StageQueueCard` / `StageProgress` per stop.
- [x] Delete `website/src/components/marketing/mocks/` and every `Feature` essay; move their
      substance into the relevant docs pages where it is not already there.
- [x] Docs restyle: Starlight on the dark tokens, sidebar and rail matching the app shell, real
      screenshots on every guide page from `e2e/site-shots.ts` (extend the script).
- [x] `verify-site.mjs`: word count < 400, hero hydration check, `--light` grep, Lighthouse ≥ 90.
- [x] `after/site/*` screenshots.
...
- [x] `PrPage.tsx`: `.finding.is-staged` head light (§4.2); commit bar as the stage: raised,
...
- [x] Extract the presentational pieces `StageScene` composes so the site and the app share them
      (no visual change from the extraction itself).
- [x] `Queue.tsx`: `--row`, single search-or-paste field, explanatory sentence removed, empty
      state with light + one line.
- [x] `StackPage.tsx`: density pass.
- [x] `e2e/review.spec.ts`: light appears on tick and leaves on untick, glow only when count > 0,
      count rolls, segmented sections, reduced-motion path.

## A3 — the rest
- [x] `Skills.tsx`: score cards → compact table; editor with line numbers and a highlighted Team
      rules section; density.
- [x] `Rollup.tsx`: single dense tile row; charts on the canvas without panel borders.
- [x] `Learnings.tsx`, `Qa.tsx`, `Integrations.tsx`, `Settings.tsx`: density and prose removal.
- [x] Delete `HowItWorks.tsx`, `bin/rs_howimg.py`, its routes and tests; docs link where the
      page was referenced.
- [x] `e2e/pages.spec.ts`: each page has no `.pagehead p`, `/how` is a 404 in the SPA, Skills
      table renders, Insights tiles in one row at 1440.

## P — proof (main session)

W, A1, A2 and A3 were finished by the main session after the lane agents stopped on a
spend limit; their checkpoints are in the merge history. Gates and numbers are in review.md.
- [x] Merge F0 → launch W, A1, A2, A3 → merge in order A1, A2, A3, W.
- [x] All gates from design §9; `after/` complete; `review.md` side by side with
      `../one-identity-redesign/after/` and the reference captures.
- [x] Tag, push, rebuild the maintainer's box, run the python suite inside the image.
