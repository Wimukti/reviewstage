...
1. Open http://localhost:8899. With [GitHub sign-in](#github-sign-in) configured the page shows **Continue with GitHub**; without it, a token form. For a solo install the token form is fine: *Create one* opens GitHub with the token name and scope pre-filled. Pick an expiry (90 days is a good default). This token acts as **you**; it is what makes every comment attributable to a person.
...
2. Fill in:
   ...
   | Authorization callback URL | **`<PUBLIC_URL>/prbot/oauth/callback`** — exact, including the `/prbot` prefix |
...
6. Restart (`docker compose up -d` again, or `systemctl restart prbot`). The login page now shows **Continue with GitHub**.
