...
cat ~/.claude-pr-bot/state/<pr>/users/<login>/agent.log     # what the agent did on one PR
cat ~/.claude-pr-bot/state/<pr>/users/<login>/review.json   # the raw agent output
cat ~/.claude-pr-bot/state/<pr>/users/<login>/status        # fetching / reviewing / done / failed
~/.claude-pr-bot/bin/run-review.sh <pr>       # run a review by hand (shared, no actor)
...
4. `PRBOT_SECRET` is regenerated, so **every outstanding Slack link is now invalid**, and every
...
- **The `pr-review` skill is a copy.** The server runs `~/.claude/skills/pr-review/`, installed
  ...
  dashboard and lives in `~/.claude-pr-bot/skills/_global.md`.)
