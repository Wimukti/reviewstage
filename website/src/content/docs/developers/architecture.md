...
| `bin/lib-limits.sh` | sourced | `MIN_FREE_MB` and `MIN_FREE_DISK_MB`, defined once so `lib-common.sh` (the job scripts) and `doctor.sh` cannot disagree about the floor. |
