...
5. Counts the PR's reviews, comments and threads with the service token before and after the run. A difference fails the job as a security incident — see [Security](/reviewstage/security/#prompt-injection-from-hostile-diffs).
