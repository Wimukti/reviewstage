...
| `MIN_FREE_DISK_MB` | `500` at a job's start; `1024` in the doctor | Refuse to start a review, a QA guide or a profiling run below this much free disk on `ROOT`, in MB (`bin/lib-common.sh`). A full volume used to look like a finished review with an empty result. `bin/doctor.sh` reads the same key but defaults it to `1024`, so the doctor FAILs at a level the job guard would still allow — set the key explicitly and both agree. |
| `RS_RETENTION_DAYS` | `30` | How long per-run `.log` files are kept before the poller's once-a-day sweep deletes them. The sweep also truncates `watch.log` and `clone.log` once either passes 8 MiB. It never touches `review.json`, `posted.json` or `approved`. See [Retention](/reviewstage/operations/troubleshooting/#what-is-on-disk-and-what-is-safe-to-delete). |
